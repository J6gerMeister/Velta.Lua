-- onyxlibrary.lua  —  Black & White edition  (fixed + 1.25x scale)
-- Bugs fixed:
...
local function makeColumnObj(sf, registry, openDD, winOptions)
	...
	local function shiftBelow(afterY,delta,animate)
		...
		local maxY=0
		for _,e in ipairs(registry[sf]) do
			local bot=e.baseY+e.extra+e.frame.AbsoluteSize.Y; if bot>maxY then maxY=bot end
		end
	...
	local function makeRow(posY,h)
		...
		row.Size=UDim2.new(1,-12,0,h); row.Position=UDim2.new(0,6,0,posY)
	...
	function col:Header(text)
		...
		local lbl=Instance.new("TextLabel"); lbl.Text=string.upper(text); lbl.Font=FONT_BOLD; lbl.TextSize=10
		lbl.TextColor3=C.accentDim; lbl.BackgroundTransparency=1; lbl.Size=UDim2.new(1,0,0,14)
		lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.ZIndex=3; lbl.Parent=wrap
		local bar=Instance.new("Frame"); bar.Size=UDim2.new(1,0,0,1); bar.Position=UDim2.new(0,0,0,16)
	...
	function col:Label(text)
		...
		local lbl=Instance.new("TextLabel"); lbl.Text=text; lbl.Font=FONT_REG; lbl.TextSize=12; lbl.TextColor3=C.textSub
	...
	function col:KeyDisplay(key)
		...
		kD.ZIndex=3; kD.Parent=sf; corner(kD,2); stroke(kD,C.borderHard,1,0)
	...
	function col:Checkbox(a1, a2, a3, a4, a5, a6, a7, a8)
		...
		local tick=Instance.new("TextLabel"); tick.Text="✓"; tick.Font=FONT_BOLD; tick.TextSize=9
		tick.TextColor3=C.bgDeep; tick.BackgroundTransparency=1; tick.Size=UDim2.fromScale(1,1)
		...
		local function applyState(v)
			tick.Visible=v
			if v then tw(box,{BackgroundColor3=C.accentMid},FAST):Play(); tw(bStroke,{Color=C.accentDim},FAST):Play(); tw(lbl,{TextColor3=C.textBright},FAST):Play()
			else tw(box,{BackgroundColor3=C.checkOff},FAST):Play(); tw(bStroke,{Color=C.borderHard},FAST):Play(); tw(lbl,{TextColor3=C.textMid},FAST):Play() end
		end
		function obj:SetValue(v) v=not not v; applyState(v); self:_fire(v) end
	...
	function col:Dropdown(a1,a2,a3,a4,a5,a6,a7,a8,a9)
		...
		local container=Instance.new("Frame"); container.Size=UDim2.new(1,-12,0,22); container.Position=UDim2.new(0,6,0,posY)
		container.BackgroundColor3=C.rowBg; container.ClipsDescendants=false; container.ZIndex=3; container.Parent=sf
		...
		local btn=Instance.new("TextButton"); btn.Size=UDim2.new(btnW,0,0,22); btn.Position=UDim2.new(btnX,0,0,0)
		btn.BackgroundColor3=C.dropBg; btn.BorderSizePixel=0; btn.Text=""; btn.AutoButtonColor=false; btn.ZIndex=6; btn.Parent=container
		corner(btn,2); local btnStroke=stroke(btn,C.borderSoft,1)
		gradient(btn,Color3.fromRGB(18,18,18),Color3.fromRGB(8,8,8),180)

		local selLbl=Instance.new("TextLabel"); selLbl.Text=options[selIdx]; selLbl.Font=FONT_REG; selLbl.TextSize=11
		selLbl.TextColor3=C.textMid; selLbl.BackgroundTransparency=1; selLbl.Size=UDim2.new(1,-20,1,0); selLbl.Position=UDim2.new(0,6,0,0)
		selLbl.TextXAlignment=Enum.TextXAlignment.Left; selLbl.ZIndex=7; selLbl.Parent=btn

		local arrow=Instance.new("TextLabel"); arrow.Text="▾"; arrow.Font=FONT_BOLD; arrow.TextSize=10
		arrow.TextColor3=C.textDim; arrow.BackgroundTransparency=1; arrow.Size=UDim2.new(0,16,1,0); arrow.Position=UDim2.new(1,-18,0,0)
		arrow.TextXAlignment=Enum.TextXAlignment.Center; arrow.ZIndex=7; arrow.Parent=btn

		local listFrame=Instance.new("Frame"); listFrame.Size=UDim2.new(btnW,0,0,0); listFrame.Position=UDim2.new(btnX,0,0,22)
		listFrame.BackgroundColor3=Color3.fromRGB(10,10,10); listFrame.BorderSizePixel=0; listFrame.ClipsDescendants=true
		listFrame.Visible=false; listFrame.ZIndex=20; listFrame.Parent=container
		corner(listFrame,2); stroke(listFrame,C.borderHard,1,0.1); gradient(listFrame,Color3.fromRGB(16,16,16),Color3.fromRGB(8,8,8),180)
		...
		local function closeDD_internal()
			...
			tw(btn,{BackgroundColor3=C.dropBg},FAST):Play(); tw(btnStroke,{Color=C.borderSoft},FAST):Play()
		...
		local function openDD_internal()
			...
			ddOpen=true; openDD.fn=closeDD_internal; listFrame.Visible=true; listFrame.Size=UDim2.new(btnW,0,0,0)
			tw(arrow,{Rotation=180,TextColor3=C.textBright},SPRING):Play()
			tw(listFrame,{Size=UDim2.new(btnW,0,0,LIST_H)},SPRING):Play()
			tw(btn,{BackgroundColor3=Color3.fromRGB(20,20,20)},FAST):Play(); tw(btnStroke,{Color=C.borderHard},FAST):Play()
		...
		for i,optText in ipairs(options) do
			local optBtn=Instance.new("TextButton"); optBtn.Size=UDim2.new(1,0,0,ITEM_H); optBtn.Position=UDim2.new(0,0,0,(i-1)*ITEM_H)
			optBtn.BackgroundColor3=(i==selIdx) and C.dropItemSel or C.dropItem
			optBtn.BackgroundTransparency=(i==selIdx) and 0 or 1; optBtn.BorderSizePixel=0; optBtn.Text=""
			optBtn.AutoButtonColor=false; optBtn.ZIndex=21; optBtn.Parent=listFrame

			local selBar=Instance.new("Frame"); selBar.Size=UDim2.new(0,2,0.55,0); selBar.Position=UDim2.new(0,2,0.22,0)
			selBar.BackgroundColor3=C.accentMid; selBar.BorderSizePixel=0; selBar.Visible=(i==selIdx); selBar.ZIndex=22; selBar.Parent=optBtn; corner(selBar,1)

			local optLbl=Instance.new("TextLabel"); optLbl.Text=optText; optLbl.Font=FONT_REG; optLbl.TextSize=11
			optLbl.TextColor3=(i==selIdx) and C.textBright or C.textMid; optLbl.BackgroundTransparency=1
			...
			optBtn.MouseEnter:Connect(function()
				if i~=selIdx then tw(optBtn,{BackgroundColor3=C.bgHover,BackgroundTransparency=0},SNAP):Play(); tw(optLbl,{TextColor3=C.textBright},SNAP):Play() end
			end)
			optBtn.MouseLeave:Connect(function()
				if i~=selIdx then tw(optBtn,{BackgroundTransparency=1},SNAP):Play(); tw(optLbl,{TextColor3=C.textMid},SNAP):Play() end
			end)
		...
		btn.MouseButton1Click:Connect(function()
			ripple(container); if ddOpen then closeDD_internal() else openDD_internal() end
		end)
		btn.MouseEnter:Connect(function() if not ddOpen then tw(btn,{BackgroundColor3=Color3.fromRGB(18,18,18)},SNAP):Play(); tw(btnStroke,{Color=C.borderHard},SNAP):Play() end end)
		btn.MouseLeave:Connect(function() if not ddOpen then tw(btn,{BackgroundColor3=C.dropBg},SNAP):Play(); tw(btnStroke,{Color=C.borderSoft},SNAP):Play() end end)
		...
		if doColorPicker then
			...
			local function openCP()
				cpOpen=true; updatePickerPos(); pickerPanel.Size=UDim2.new(1,0,0,0); pickerPanel.Visible=true
				tw(swatchStroke,{Color=C.accentMid},FAST):Play()
				tw(pickerPanel,{Size=UDim2.new(1,0,0,PICKER_H)},SPRING):Play()
				tw(container,{Size=UDim2.new(1,-12,0,containerH())},MED):Play()
				shiftBelow(posY,PICKER_H+2,true)
	...
	function col:Slider(a1,a2,a3,a4,a5,a6)
		...
		local lbl=Instance.new("TextLabel"); lbl.Text=tostring(labelText); lbl.Font=FONT_REG; lbl.TextSize=12; lbl.TextColor3=C.textMid
		lbl.BackgroundTransparency=1; lbl.Size=UDim2.new(0.42,0,1,0); lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.ZIndex=4; lbl.Parent=row

		local valLbl=Instance.new("TextLabel"); valLbl.Text=tostring(default); valLbl.Font=FONT_REG; valLbl.TextSize=10; valLbl.TextColor3=C.accentMid
		...
		local track=Instance.new("Frame"); track.Size=UDim2.new(0.42,0,0,3); track.Position=UDim2.new(0.43,0,0.5,-1)
		...
		local fill=Instance.new("Frame"); fill.Size=UDim2.new(pct,0,1,0); fill.BackgroundColor3=C.sliderFill
		...
		local knob=Instance.new("TextButton"); knob.Size=UDim2.new(0,11,0,11); knob.Position=UDim2.new(pct,-5,0.5,-5)
		knob.BackgroundColor3=C.knob; knob.BorderSizePixel=0; knob.Text=""; knob.AutoButtonColor=false; knob.ZIndex=6; knob.Parent=track; corner(knob,6)
		stroke(knob,C.borderHard,1.5,0)

		local function applyValue(v)
			v=math.clamp(v,minVal,maxVal); local p=(v-minVal)/math.max(maxVal-minVal,1)
			tw(fill,{Size=UDim2.new(p,0,1,0)},SNAP):Play(); tw(knob,{Position=UDim2.new(p,-5,0.5,-5)},SNAP):Play()
	...
	function col:Keybind(a1,a2,a3,a4)
		...
		local lbl=Instance.new("TextLabel"); lbl.Text=tostring(labelText); lbl.Font=FONT_REG; lbl.TextSize=12; lbl.TextColor3=C.textMid
		lbl.BackgroundTransparency=1; lbl.Size=UDim2.new(0.55,0,1,0); lbl.TextXAlignment=Enum.TextXAlignment.Left; lbl.ZIndex=4; lbl.Parent=row

		local kBtn=Instance.new("TextButton"); kBtn.Size=UDim2.new(0.4,0,0.72,0); kBtn.Position=UDim2.new(0.57,0,0.14,0)
		kBtn.BackgroundColor3=C.bgRaised; kBtn.BorderSizePixel=0; kBtn.Text=obj.Value
		kBtn.Font=FONT_BOLD; kBtn.TextSize=10; kBtn.TextColor3=C.textMid; kBtn.AutoButtonColor=false; kBtn.ZIndex=4; kBtn.Parent=row; corner(kBtn,2)
		local kS=stroke(kBtn,C.borderHard,1,0.1)

		local picking=false
		function obj:SetValue(k) k=k or "None"; kBtn.Text=k; tw(kBtn,{BackgroundColor3=C.bgPress},SNAP):Play(); task.delay(0.18,function() tw(kBtn,{BackgroundColor3=C.bgRaised},FAST):Play() end); self:_fire(k) end

		kBtn.MouseButton1Click:Connect(function()
			if picking then return end; picking=true; kBtn.Text="..."
			tw(kBtn,{BackgroundColor3=C.bgHover},SNAP):Play(); tw(kS,{Color=C.accentDim},SNAP):Play()
	...
	function col:PairedCheckbox(a1,a2,a3,a4,a5,a6,a7,a8)
		...
		local function makeMini(text,xScale,obj)
			local box=Instance.new("TextButton"); box.Size=UDim2.new(0,12,0,12); box.Position=UDim2.new(xScale,3,0.5,-6)
			box.BackgroundColor3=obj.Value and C.accentMid or C.checkOff; box.BorderSizePixel=0
			box.Text=""; box.AutoButtonColor=false; box.ZIndex=4; box.Parent=row; corner(box,2)
			local bS=stroke(box,obj.Value and C.accentDim or C.borderHard,1)
			local tick=Instance.new("TextLabel"); tick.Text="✓"; tick.Font=FONT_BOLD; tick.TextSize=8; tick.TextColor3=C.bgDeep
			tick.BackgroundTransparency=1; tick.Size=UDim2.fromScale(1,1)
			tick.TextXAlignment=Enum.TextXAlignment.Center; tick.TextYAlignment=Enum.TextYAlignment.Center
			tick.Visible=obj.Value; tick.ZIndex=5; tick.Parent=box
			local ml=Instance.new("TextLabel"); ml.Text=tostring(text); ml.Font=FONT_REG; ml.TextSize=11
			ml.TextColor3=obj.Value and C.textBright or C.textMid; ml.BackgroundTransparency=1
	...
	function col:ExpandableCheckbox(a1,a2,a3,a4,a5)
		...
		local tick=Instance.new("TextLabel"); tick.Text="✓"; tick.Font=FONT_BOLD; tick.TextSize=9; tick.TextColor3=C.bgDeep
		tick.BackgroundTransparency=1; tick.Size=UDim2.fromScale(1,1)
		tick.TextXAlignment=Enum.TextXAlignment.Center; tick.TextYAlignment=Enum.TextYAlignment.Center
		tick.Visible=obj.Value; tick.ZIndex=5; tick.Parent=box
		...
		local expArrow=Instance.new("TextLabel"); expArrow.Text="▾"; expArrow.Font=FONT_BOLD; expArrow.TextSize=10
		expArrow.TextColor3=C.textDim; expArrow.BackgroundTransparency=1; expArrow.Size=UDim2.new(0,16,1,0); expArrow.Position=UDim2.new(1,-18,0,0)
		expArrow.TextXAlignment=Enum.TextXAlignment.Center; expArrow.ZIndex=4; expArrow.Parent=row

		local subPanel=Instance.new("Frame"); subPanel.Size=UDim2.new(1,-12,0,0); subPanel.Position=UDim2.new(0,6,0,posY+26)
		...
		local subSF=Instance.new("ScrollingFrame"); subSF.Size=UDim2.fromScale(1,1); subSF.BackgroundTransparency=1; subSF.BorderSizePixel=0
		subSF.ScrollBarThickness=2; subSF.ScrollBarImageColor3=C.accentDim; subSF.CanvasSize=UDim2.new(0,0,0,2000); subSF.ZIndex=2; subSF.Parent=subPanel
		...
		local subH=math.min(subColObj._y+8,220); subSF.CanvasSize=UDim2.new(0,0,0,subColObj._y+8)

		local expanded=false
		local function openSub()
			expanded=true; subPanel.Visible=true; subPanel.Size=UDim2.new(1,-12,0,0)
			tw(subPanel,{Size=UDim2.new(1,-12,0,subH)},SPRING):Play(); tw(expArrow,{Rotation=180},MED):Play(); shiftBelow(posY,subH+2)
		end
		local function closeSub()
			expanded=false; tw(subPanel,{Size=UDim2.new(1,-12,0,0)},MED):Play(); tw(expArrow,{Rotation=0},MED):Play()
			task.delay(0.26,function() subPanel.Visible=false end); shiftBelow(posY,-(subH+2))
		end
		...
		box.MouseButton1Click:Connect(function() ripple(row); obj:SetValue(not obj.Value) end)
		local arBtn=Instance.new("TextButton"); arBtn.Size=UDim2.new(0,24,1,0); arBtn.Position=UDim2.new(1,-26,0,0)
		arBtn.BackgroundTransparency=1; arBtn.Text=""; arBtn.ZIndex=6; arBtn.Parent=row
		arBtn.MouseButton1Click:Connect(function() if not obj.Value then return end; if expanded then closeSub() else openSub() end end)
		row.MouseEnter:Connect(function() tw(row,{BackgroundColor3=C.bgHover},SNAP):Play() end)
		row.MouseLeave:Connect(function() tw(row,{BackgroundColor3=C.rowBg},SNAP):Play() end)
...
local function makeTabObj(panel, registry, openDD, winOptions)
	...
	local function makeScrollCol(size,pos)
		local sf=Instance.new("ScrollingFrame"); sf.Size=size; sf.Position=pos or UDim2.new(0,0,0,0)
		sf.BackgroundTransparency=1; sf.BorderSizePixel=0; sf.ScrollBarThickness=2; sf.ScrollBarImageColor3=C.accentDim
		sf.CanvasSize=UDim2.new(0,0,0,2000); sf.ZIndex=2; sf.Parent=panel; return sf
	end
	function tabObj:TwoColumn()
		local lSF=makeScrollCol(UDim2.new(0.5,-1,1,0)); local rSF=makeScrollCol(UDim2.new(0.5,-1,1,0),UDim2.new(0.5,1,0,0))
		...
		return makeColumnObj(lSF,registry,openDD,winOptions), makeColumnObj(rSF,registry,openDD,winOptions)
	end
	function tabObj:SingleColumn()
		local sf=makeScrollCol(UDim2.fromScale(1,1)); return makeColumnObj(sf,registry,openDD,winOptions)
	end
	return tabObj
end
...
function OnyxiteLib.new(config)
	...
	local tLbl=Instance.new("TextLabel"); tLbl.Text=config.Title or "Onyxite"; tLbl.Font=FONT_BOLD; tLbl.TextSize=14
	...
	local function makeWinBtn(xOff,glyph,hBg,hTxt)
		...
		b.BackgroundColor3=Color3.fromRGB(16,16,16); b.BorderSizePixel=0; b.Text=glyph; b.Font=FONT_BOLD; b.TextSize=14
	...
	local pLbl=Instance.new("TextLabel"); pLbl.Text=string.upper(config.Title or "ONYXITE"); pLbl.Font=FONT_BOLD; pLbl.TextSize=10
	...
	dTitle.Text="CLOSE "..(string.upper(config.Title or "ONYXITE?")); dTitle.TextXAlignment=Enum.TextXAlignment.Left; dTitle.ZIndex=93; dTitle.Parent=cDlg
	...
	local dDiv=Instance.new("Frame"); dDiv.Size=UDim2.new(1,-24,0,1); dDiv.Position=UDim2.new(0,12,0,96); dDiv.BackgroundColor3=C.borderFaint; dDiv.BorderSizePixel=0; dDiv.ZIndex=93; dDiv.Parent=cDlg
	local function mDB(x,w,t,bg,tc,sc)
		local b=Instance.new("TextButton"); b.Size=UDim2.new(0,w,0,32); b.Position=UDim2.new(0,x,1,-44)
		b.BackgroundColor3=bg; b.BorderSizePixel=0; b.Text=t; b.TextColor3=tc; b.TextTransparency=1; b.TextSize=11
		b.Font=FONT_REG; b.AutoButtonColor=false; b.ZIndex=93; b.Parent=cDlg; corner(b,2); stroke(b,sc,1,0.4); return b
	end
	...
	local function closeDialog()
		tw(bOver,{BackgroundTransparency=1},MED):Play(); tw(dTitle,{TextTransparency=1},FAST):Play()
		tw(dMsg,{TextTransparency=1},FAST):Play(); tw(cancelBtn,{TextTransparency=1},FAST):Play(); tw(confirmBtn,{TextTransparency=1},FAST):Play()
	...
	local sLA=Instance.new("Frame"); sLA.Size=UDim2.new(1,0,0,44); sLA.BackgroundColor3=Color3.fromRGB(10,10,10); sLA.BorderSizePixel=0; sLA.ZIndex=5; sLA.Parent=sidebar; corner(sLA,2)
	...
	local sLT=Instance.new("TextLabel"); sLT.Text=config.Creator or "Onyxite"; sLT.Font=FONT_SCI; sLT.TextSize=11
	...
	local sTBtn=Instance.new("TextButton"); sTBtn.Size=UDim2.new(1,0,0,26); sTBtn.Position=UDim2.new(0,0,1,-(PROFILE_H+26))
	...
	local profLine=Instance.new("Frame"); profLine.Size=UDim2.new(1,0,0,1); profLine.BackgroundColor3=C.profileLine; profLine.BorderSizePixel=0; profLine.ZIndex=7; profLine.Parent=profileCard
	...
	local avatarFrame=Instance.new("Frame"); avatarFrame.Size=UDim2.new(0,40,0,40); avatarFrame.Position=UDim2.new(0,12,0.5,-20)
	...
	local avatarImg=Instance.new("ImageLabel"); avatarImg.Size=UDim2.fromScale(1,1); avatarImg.BackgroundTransparency=1
	avatarImg.Image="rbxthumb://type=AvatarHeadShot&id="..tostring(player.UserId).."&w=150&h=150"; avatarImg.ZIndex=9; avatarImg.Parent=avatarFrame; corner(avatarImg,5)

	local profName=Instance.new("TextLabel"); profName.Size=UDim2.new(1,-62,0,18); profName.Position=UDim2.new(0,58,0,12)
	...
	local profUser=Instance.new("TextLabel"); profUser.Size=UDim2.new(1,-62,0,14); profUser.Position=UDim2.new(0,58,0,32)
	...
	for i,def in ipairs(tabDefs) do
		...
		local panel=Instance.new("CanvasGroup"); panel.Size=UDim2.fromScale(1,1)
		panel.BackgroundTransparency=1; panel.Visible=false; panel.GroupTransparency=0; panel.ZIndex=2; panel.Parent=cArea
		...
		local iL=Instance.new("TextLabel"); iL.Text=def.Icon or "·"; iL.Font=FONT_REG; iL.TextSize=15
		...
		btn.MouseEnter:Connect(function()
			if win._activeTab~=cn then tw(btn,{BackgroundColor3=C.tabHover},SNAP):Play(); tw(iL,{TextColor3=C.textSub},SNAP):Play(); tw(lbl,{TextColor3=C.textSub},SNAP):Play() end
		end)
		btn.MouseLeave:Connect(function()
			if win._activeTab~=cn then tw(btn,{BackgroundColor3=C.tabInact},SNAP):Play(); tw(iL,{TextColor3=C.textDim},SNAP):Play(); tw(lbl,{TextColor3=C.textDim},SNAP):Play() end
		end)
	end
