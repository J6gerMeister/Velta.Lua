...
--   ColorPicker is NOT a standalone row.
--   Pass colorPicker=true to Dropdown() and a small colored square swatch
--   appears between the label text and the dropdown button, on the same row:
--
--       Outline Box  [■]           None ▾
--                    ^swatch cube
--
--   Clicking the swatch expands the color picker panel inline, directly
--   below the dropdown row.  The dropdown list can still open separately.
--   While the picker is open the swatch border is RGB.
--   When closed the swatch border is plain dim and shows the chosen color.
--
--   SV SQUARE:
--     Background  = pure hue  (e.g. red)
--     Layer 1     = white gradient, opaque-left → transparent-right
--                   (result: left col = white, right col = hue)
--     Layer 2     = black gradient, transparent-top → opaque-bottom
--                   (result: top-left = white, top-right = hue, bottom = black)
--
-- SIGNATURE:
--   col:Dropdown(labelText, options, default, callback,
--                colorPicker, defaultColor, defaultOpacity, colorCallback)
--
--   col:ExpandableCheckbox(labelText, default, callback, subBuilder)
--     subBuilder(subCol)  — subCol supports all col: methods
...
local function stroke(inst, col, thick, trans)
	local s=Instance.new("UIStroke"); s.Color=col or C.border; s.Thickness=thick or 1; s.Transparency=trans or 0; s.Parent=inst; return s
end
...
-- ============================================================
--  buildColorPicker  (reusable inner factory)
--  Places all widgets inside `parent` starting at local Y=startY.
--  Returns: panelFrame, fnGetColor, fnGetOpacity, totalHeight
-- ============================================================
local function buildColorPicker(parent, startY, defColor, defOpacity, colorCb)
	...
	-- ── tuneable layout (pixels) ────────────────────────────
	local PAD    = 5     -- inner padding on all sides
	local HUE_H  = 10   -- hue bar height
	local SV_H   = 58   -- SV square height
	local SV_WF  = 0.62 -- fraction of panel width for SV square
	local PREV_G = 4    -- gap between SV and preview
	local SL_H   = 18   -- each slider row height (track sits in center)
	local TOTAL_H = PAD + HUE_H + PAD + SV_H + PAD + SL_H + PAD + SL_H + PAD
	-- ────────────────────────────────────────────────────────

	local panel = Instance.new("Frame")
	panel.Size             = UDim2.new(1, 0, 0, TOTAL_H)
	panel.Position         = UDim2.new(0, 0, 0, startY)
	...
	hg.Color = makeRainbowSeq(); hg.Rotation = 0; hg.Parent = hueBar
	...
	svBox.Size             = UDim2.new(SV_WF, -PAD - PREV_G/2, 0, SV_H)
	svBox.Position         = UDim2.new(0, PAD, 0, svY)
	svBox.BackgroundColor3 = Color3.fromHSV(curH,1,1)  -- pure hue (right-side colour)
	svBox.BorderSizePixel  = 0; svBox.ZIndex=9
	svBox.ClipsDescendants = true; svBox.Parent=panel
	corner(svBox,2)

	-- Layer 1: white → transparent  horizontally  (left=white, right shows hue)
	...
	-- Layer 2: transparent → black  vertically  (top=transparent, bottom=black)
	...
	svCursor.Size=UDim2.new(0,9,0,9)
	...
	prevFrame.Size=UDim2.new(1-SV_WF,-PAD-PREV_G/2,0,SV_H)
	prevFrame.Position=UDim2.new(SV_WF,PREV_G/2,0,svY)
	...
	brTrack.Size=UDim2.new(1,-PAD*2,0,4); brTrack.Position=UDim2.new(0,PAD,0,brMid-2)
	...
	brLbl.BackgroundTransparency=1; brLbl.Size=UDim2.new(0,8,0,SL_H); brLbl.Position=UDim2.new(0,PAD,0,brY)
	...
	brVal.BackgroundTransparency=1; brVal.Size=UDim2.new(0,28,0,SL_H); brVal.Position=UDim2.new(1,-PAD-28,0,brY)
	...
	opTrack.Size=UDim2.new(1,-PAD*2,0,4); opTrack.Position=UDim2.new(0,PAD,0,opMid-2)
	...
	opLbl.BackgroundTransparency=1; opLbl.Size=UDim2.new(0,8,0,SL_H); opLbl.Position=UDim2.new(0,PAD,0,opY)
	...
	opVal.BackgroundTransparency=1; opVal.Size=UDim2.new(0,28,0,SL_H); opVal.Position=UDim2.new(1,-PAD-28,0,opY)
	...
	local function refreshAll()
		...
		-- update white gradient color (should stay white but update for hue)
		svCursor.Position=UDim2.new(curS,-4,1-curV,-4)
		hueCursor.Position=UDim2.new(curH,-1,0,-2)
	...
	UIS.InputEnded:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 then hueDrag=false end end)
	...
	UIS.InputEnded:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 then svDrag=false end end)
	...
	UIS.InputEnded:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 then brDrag=false end end)
	...
	UIS.InputEnded:Connect(function(inp) if inp.UserInputType==Enum.UserInputType.MouseButton1 then opDrag=false end end)

	return panel, getColor, getOpacity, TOTAL_H
end
...
local function makeColumnObj(sf, registry, openDD)
	...
	local function shiftBelow(afterY, delta)
		for _,e in ipairs(registry[sf]) do
			if e.baseY>afterY then
				...
				e.frame.Position=UDim2.new(e.frame.Position.X.Scale,e.frame.Position.X.Offset,0,e.baseY+e.extra)
			end
		end
	end
	...
	function col:Header(text)
		...
		lbl.BackgroundTransparency=1; lbl.Size=UDim2.new(1,0,0,14); lbl.TextXAlignment=Enum.TextXAlignment.Left
	...
	-- ================================================================
	--  Dropdown  — with optional inline color swatch
	-- ================================================================
	function col:Dropdown(labelText, options, default, callback,
	                      doColorPicker, defColor, defOpacity, colorCb)
		local posY=self._y
		local COUNT=#options; local LIST_H=COUNT*ITEM_H
		local ddOpen=false; local cpOpen=false; local cpH=0

		-- container (grows as things expand)
		...
		gradient(container,C.rowBgLight,C.rowBg,180); regItem(container,posY)

		-- ── swatch (only when colorPicker=true) ──────────────
		-- It sits between the label and the dropdown button.
		-- Label occupies ~44% of width minus 20px for swatch.
		-- Swatch occupies 16×16 px right after label.
		local SWATCH_OFFSET = doColorPicker and 20 or 0

		if labelText ~= "" then
			local lbl=Instance.new("TextLabel")
			lbl.Text=labelText; lbl.Font=FONT_REG; lbl.TextSize=12; lbl.TextColor3=C.text
			lbl.BackgroundTransparency=1
			lbl.Size=UDim2.new(0.44,-SWATCH_OFFSET,0,22)
		...
		-- list frame (dropdown options)
		local listFrame=Instance.new("Frame")
		listFrame.Size=UDim2.new(btnW,0,0,0); listFrame.Position=UDim2.new(btnX,0,0,24)
		listFrame.BackgroundColor3=Color3.fromRGB(16,16,16); listFrame.BorderSizePixel=0
		listFrame.ClipsDescendants=true; listFrame.Visible=false; listFrame.ZIndex=20; listFrame.Parent=container
		...
		-- helper: compute current total container height
		local function containerH()
			return 22 + (ddOpen and LIST_H or 0) + (cpOpen and (cpH+2) or 0)
		end
		...
		for i,optText in ipairs(options) do
			...
			optLbl.TextColor3=(i==selIdx) and rgbColor or C.text; optLbl.BackgroundTransparency=1
		...
		btn.MouseButton1Click:Connect(function() if ddOpen then closeDD() else openDD_fn() end end)
		btn.MouseEnter:Connect(function() if not ddOpen then tw(btn,{BackgroundColor3=Color3.fromRGB(22,22,22)}):Play(); tw(btnStroke,{Color=C.borderBt}):Play() end end)
		btn.MouseLeave:Connect(function() if not ddOpen then tw(btn,{BackgroundColor3=C.dropBg}):Play(); tw(btnStroke,{Color=C.border}):Play() end end)

		-- ── color picker (swatch click expands panel) ─────────
		if doColorPicker then
			-- Build picker; it lives inside the container, starting at Y=24
			-- (below the 22px row + 2px gap); width fills container.
			local pickerPanel, getC, getOp, pickerH =
				buildColorPicker(container, 24, defColor, defOpacity, function(c,op)
			...
			-- position picker below dropdown list when both open;
			-- the panel's Position.Y must track the current list height
			local function updatePickerPos()
				pickerPanel.Position=UDim2.new(0,0,0,24+(ddOpen and LIST_H or 0))
			end

			local origCloseDD=closeDD
			closeDD=function()
				origCloseDD()
				updatePickerPos()
			end
			...
			local function openCP()
				...
				tw(pickerPanel,{Size=UDim2.new(1,0,0,cpH)},MED):Play()
				container.Size=UDim2.new(1,-12,0,containerH())
				shiftBelow(posY,cpH+2)
			end
	...
	function col:Slider(labelText, minVal, maxVal, default, callback)
		...
		local lbl=Instance.new("TextLabel")
		lbl.Text=labelText; lbl.Font=FONT_REG; lbl.TextSize=12; lbl.TextColor3=C.text
		lbl.BackgroundTransparency=1; lbl.Size=UDim2.new(0.42,0,1,0); lbl.TextXAlignment=Enum.TextXAlignment.Left
	...
	function col:Keybind(labelText, key)
		...
		local lbl=Instance.new("TextLabel")
		lbl.Text=labelText; lbl.Font=FONT_REG; lbl.TextSize=12; lbl.TextColor3=C.text
		lbl.BackgroundTransparency=1; lbl.Size=UDim2.new(0.55,0,1,0); lbl.TextXAlignment=Enum.TextXAlignment.Left
		lbl.ZIndex=4; lbl.Parent=row
	...
	function col:Label(text)
		...
		lbl.BackgroundTransparency=1; lbl.Size=UDim2.fromScale(1,1); lbl.TextXAlignment=Enum.TextXAlignment.Left
	...
	function col:PairedCheckbox(lL, dL, lR, dR, cbL, cbR)
		...
		makeMini(lL,0,dL,cbL); makeMini(lR,0.5,dR,cbR)
...
local function makeTabObj(panel, registry, openDD)
	...
	local function makeScrollCol(size,pos)
		...
		sf.Size=size; sf.Position=pos or UDim2.new(0,0,0,0); sf.BackgroundTransparency=1
		sf.BorderSizePixel=0; sf.ScrollBarThickness=2; sf.ScrollBarImageColor3=rgbColor
	...
	function tabObj:SingleColumn()
		local sf=makeScrollCol(UDim2.fromScale(1,1)); return makeColumnObj(sf,registry,openDD)
	end
	return tabObj
end
...
function VeltaLib.new(config)
	...
	local WIN_W=config.Width or 880; local WIN_H=config.Height or 530
	local BORDER=5; local TITLEBAR_H=32; local SIDEBAR_OW=140; local SIDEBAR_CW=36
	local WIN_MIN_W=600; local WIN_MIN_H=380; local sidebarOpen=true; local menuVisible=true

	local player=Players.LocalPlayer; local guiParent=player:WaitForChild("PlayerGui")
	...
	corner(outerFrame,0)
	local sg=Instance.new("UIGradient"); sg.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,C.shellLight),ColorSequenceKeypoint.new(0.5,C.shellMid),ColorSequenceKeypoint.new(1,C.shellDark)}); sg.Rotation=135; sg.Parent=outerFrame
	stroke(outerFrame,Color3.fromRGB(80,80,80),1,0)

	local main=Instance.new("Frame"); main.Name="Main"; main.Size=UDim2.new(1,-BORDER*2,1,-BORDER*2)
	main.Position=UDim2.new(0,BORDER,0,BORDER); main.BackgroundColor3=C.bgTop; main.BorderSizePixel=0
	main.ZIndex=2; main.ClipsDescendants=false; main.Parent=outerFrame
	corner(main,0); gradient(main,C.bgTop,C.bgBot,160); stroke(main,C.borderBt,1,0)

	local topAccent=Instance.new("Frame"); topAccent.Size=UDim2.new(0,80,0,2); topAccent.BackgroundColor3=rgbColor
	topAccent.BorderSizePixel=0; topAccent.ZIndex=6; topAccent.Parent=main; corner(topAccent,1); bindRGB(topAccent,"BackgroundColor3")

	local titleBar=Instance.new("Frame"); titleBar.Name="TitleBar"; titleBar.Size=UDim2.new(1,0,0,TITLEBAR_H)
	titleBar.BackgroundColor3=C.panel; titleBar.BorderSizePixel=0; titleBar.ZIndex=4; titleBar.Parent=main
	...
	sDot.BackgroundColor3=rgbColor; sDot.BorderSizePixel=0; sDot.ZIndex=6; sDot.Parent=titleBar; corner(sDot,3); bindRGB(sDot,"BackgroundColor3")
	...
	UIS.InputChanged:Connect(function(inp)
		if pDrag and inp.UserInputType==Enum.UserInputType.MouseMovement then
			local d=inp.Position-pDS; rPill.Position=UDim2.new(pSP.X.Scale,pSP.X.Offset+d.X,pSP.Y.Scale,pSP.Y.Offset+d.Y) end end)
	...
	dMsg.TextTransparency=1; dMsg.TextWrapped=true; dMsg.Text="Are you sure you want to close the menu?\nRe-execute the script to reopen it."
	...
	local function minimize()
		...
		task.delay(0.08,function() outerFrame.Visible=false; rPill.Position=UDim2.new(0.5,-60,0,-40); rPill.Visible=true; tw(rPill,{Position=UDim2.new(0.5,-60,0,10)},SLOW):Play() end)
	end
	...
	UIS.InputBegan:Connect(function(inp,gp) if gp then return end; if inp.KeyCode==Enum.KeyCode.Insert then if menuVisible then minimize() else restore() end end end)
	...
	local sidebar=Instance.new("Frame"); sidebar.Name="Sidebar"; sidebar.Size=UDim2.new(0,SIDEBAR_OW,1,-TITLEBAR_H)
	sidebar.Position=UDim2.new(0,0,0,TITLEBAR_H); sidebar.BackgroundColor3=C.sidebarBg
	sidebar.BorderSizePixel=0; sidebar.ZIndex=4; sidebar.ClipsDescendants=true; sidebar.Parent=main
	...
	sLA.BorderSizePixel=0; sLA.ZIndex=5; sLA.Parent=sidebar; corner(sLA,0); gradient(sLA,Color3.fromRGB(30,30,30),Color3.fromRGB(12,12,12),170)
	...
	local function showTab(name)
		...
		for _,d in ipairs(win._tabButtons) do
			...
			if active then if not d._iR then d._iR=bindRGB(d.iL,"TextColor3") end
			else removeRGB(d._iR); d._iR=nil; d.iL.TextColor3=C.textDim end
			if active then if not d._aR then d._aR=bindRGB(d.ac,"BackgroundColor3") end
			else removeRGB(d._aR); d._aR=nil; d.ac.BackgroundColor3=C.border end
			d.ac.Visible=active; tw(d.lbl,{TextColor3=active and C.textBright or C.textDim}):Play()
		end
		win._activeTab=name
	end
	...
	local TAB_H=34; local tabDefs=config.Tabs or {}
	if #tabDefs>0 then win._activeTab=tabDefs[1].Name end

	for i,def in ipairs(tabDefs) do
		local yPos=40+(i-1)*TAB_H
		local panel=Instance.new("Frame"); panel.Size=UDim2.fromScale(1,1); panel.BackgroundTransparency=1
		panel.Visible=false; panel.ZIndex=2; panel.Parent=cArea; win._tabPanels[def.Name]=panel
		...
		iL.TextColor3=(def.Name==win._activeTab) and rgbColor or C.textDim; iL.BackgroundTransparency=1
		iL.Size=UDim2.new(0,SIDEBAR_CW,1,0); iL.TextXAlignment=Enum.TextXAlignment.Center; iL.ZIndex=7; iL.Parent=btn
		...
		btn.MouseEnter:Connect(function() if win._activeTab~=cn then tw(btn,{BackgroundColor3=C.panelHover}):Play(); tw(iL,{TextColor3=C.text}):Play(); tw(lbl,{TextColor3=C.text}):Play() end end)
		btn.MouseLeave:Connect(function() if win._activeTab~=cn then tw(btn,{BackgroundColor3=C.tabInact}):Play(); tw(iL,{TextColor3=C.textDim}):Play(); tw(lbl,{TextColor3=C.textDim}):Play() end end)
	end
